...
- **Area capture** (`Alt+Shift+S` by default — the primary shortcut) — drag
  over exactly what you want, macOS ⌘⇧4-style; the shot is cropped to your
  selection so no junk reaches your AI chat. Esc cancels.
- **Instant visible capture** (`Alt+Shift+A` by default) grabs the whole
  visible tab area with no selection step
